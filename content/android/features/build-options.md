...
With this file in the root of your project, any future compilation will leverage it, speeding up the compilation by 
skipping un-swazzleable elements of you project.

### Disabling the swazzling cache

If you want to go back to revert to the full swazzling process, you can either delete the file manually or run the 
Gradle task `cleanEmbraceSwazzlingRulesFor<Variant>`, e.g. `cleanEmbraceSwazzlingRulesForDebug` or 
...
To reduce the scan time, when you have generated `embrace-swazzling-rules.json` file the Gradle plugin will use the 
