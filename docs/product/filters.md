...
| Method<br /><code class="filters-table__key">method</code> | Method where the ANR occurred. | String | — |
...
| Has Connection Error<br /><code class="filters-table__key">has_connection_error</code> | Whether the session contains a network connection error. | Boolean | — |
...
| User Flow Outcome Reason<br /><code class="filters-table__key">user_flow_outcome_reason</code> | Reason for the user flow outcome. | Choice (String) | Choices: `app_exit`, `timeout`, `new_user_flow_started`, `crash` |
