...
```
 ...
 ## Alert Rate Limiting
  ...
  When a rate limit is reached, additional alerts are suppressed until the time window resets. **Important**: Even if a notification is suppressed, all triggered alerts are still recorded and visible in your Alert History
   in the dashboard.
