...
```swift
func loadData() {
    let span = EmbraceIO.shared.createSpan(name: "data_loading")
...
```swift
func renderFeed() {
    let renderSpan = EmbraceIO.shared.createSpan(name: "feed_rendering")

    // 1. Load data
    let dataSpan = EmbraceIO.shared.createSpan(name: "feed_data_loading", parentSpan: renderSpan)
    let feedItems = loadFeedData()
    dataSpan?.setAttribute(key: "item_count", value: "\(feedItems.count)")
    dataSpan?.end()

    // 2. Process images
    let imageSpan = EmbraceIO.shared.createSpan(name: "feed_image_processing", parentSpan: renderSpan)
    processFeedImages(feedItems)
    imageSpan?.end()

    // 3. Update UI
    let uiSpan = EmbraceIO.shared.createSpan(name: "feed_ui_update", parentSpan: renderSpan)
...
func loadUserProfile() {
    let span = EmbraceIO.shared.createSpan(name: "user_profile_loading")
...
func checkoutProcess() {
    ...
    let checkoutSpan = EmbraceIO.shared.createSpan(name: "checkout_flow")
...
func validateCart(completion: @escaping (Bool) -> Void) {
    let span = EmbraceIO.shared.createSpan(name: "cart_validation")
...
func processPayment(completion: @escaping (Bool) -> Void) {
    let span = EmbraceIO.shared.createSpan(name: "payment_processing")
...
func confirmOrder(completion: @escaping (Bool) -> Void) {
    let span = EmbraceIO.shared.createSpan(name: "order_confirmation")
...
class ProductListViewController: UIViewController {
    override func viewDidLoad() {
        super.viewDidLoad()
        EmbraceIO.shared.log("ProductList viewDidLoad started", severity: .info)

        // Setup UI
        setupUI()

        EmbraceIO.shared.log("ProductList viewDidLoad completed", severity: .info)
    }

    override func viewWillAppear(_ animated: Bool) {
        super.viewWillAppear(animated)

        let span = EmbraceIO.shared.createSpan(name: "product_list_loading")
    ...
    private func updateUI(with products: [Product]) {
        let span = EmbraceIO.shared.createSpan(name: "product_list_ui_update")
...
if let userSubscriptionTier = userManager.currentUser?.subscriptionTier {
    EmbraceIO.shared.setProperty(key: "subscription_tier", value: userSubscriptionTier, lifespan: .session)
}

// Track app configuration that impacts performance
EmbraceIO.shared.setProperty(key: "image_quality", value: appSettings.imageQuality.rawValue, lifespan: .session)
EmbraceIO.shared.setProperty(key: "animations_enabled", value: appSettings.animationsEnabled ? "true" : "false", lifespan: .session)
```
...
func trackScrollPerformance() {
    let scrollSpan = EmbraceIO.shared.createSpan(name: "feed_scrolling")
