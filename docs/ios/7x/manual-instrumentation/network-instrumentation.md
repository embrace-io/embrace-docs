...
class GRPCInstrumentation {
    func recordGRPCRequest(
        ...
        // Record the completed span by supplying both start and end times
        EmbraceIO.shared.createSpan(
            name: "POST /\(service)/\(method)",
            parentSpan: nil,
            type: .networkRequest,
            status: error != nil ? .error : .unset,
            startTime: startTime,
            endTime: endTime,
            events: [],
...
```swift
func startGRPCSpan(service: String, method: String) -> EmbraceSpan? {
    let grpcURL = "grpc://\(service)/\(method)"
    let attributes = [
        "url.full": grpcURL,
    ...
    return EmbraceIO.shared.createSpan(
        name: "POST /\(service)/\(method)",
        type: .networkRequest,
        attributes: attributes
    )
...
let userSpan = EmbraceIO.shared.createSpan(
    name: "GET /api/user",
    parentSpan: parentSpan,
    type: .networkRequest
)

let preferencesSpan = EmbraceIO.shared.createSpan(
...
class WebSocketInstrumentation {
    private var connectionSpan: EmbraceSpan?

    func startConnection(url: String) {
        connectionSpan = EmbraceIO.shared.createSpan(
