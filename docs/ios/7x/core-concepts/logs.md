---
title: Logs
description: Understanding logs in the Embrace iOS SDK 7.x
sidebar_position: 3
---
...
- **severity**: The [EmbraceLogSeverity](https://github.com/embrace-io/embrace-apple-sdk/blob/main/Sources/EmbraceCommonInternal/Models/LogSeverity.swift) of the event (e.g., info, warn, error)
...
class AuthenticationManager {
    func login(username: String, password: String) async throws -> AuthResult {
        ...
        do {
            ...
            // Update user context
            EmbraceIO.shared.userIdentifier = result.userId
            EmbraceIO.shared.addPersona("authenticated", lifespan: .process)
...
class PerformanceLogger {
    func trackCriticalOperation<T>(
        ...
        do {
            ...
            let severity: EmbraceLogSeverity = duration > expectedDuration ? .warn : .info
    ...
    func logDatabaseOperation<T>(
        ...
        do {
            ...
            EmbraceIO.shared.log(
                ...
                severity: duration > 1.0 ? .warn : .debug, // Warn if over 1 second
                attributes: [
...
class UIPerformanceLogger {
    func logViewControllerLifecycle(
        ...
        let severity: EmbraceLogSeverity = duration > 0.5 ? .warn : .debug
    ...
    func logMainThreadBlock(duration: TimeInterval, operation: String) {
        if duration > 0.1 { // Log if main thread blocked for more than 100ms
            EmbraceIO.shared.log(
                "Main thread blocked",
                severity: duration > 0.5 ? .error : .warn,
                attributes: [
