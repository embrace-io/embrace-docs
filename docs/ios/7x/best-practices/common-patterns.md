...
description: Common implementation patterns for the Embrace iOS SDK 7.x
sidebar_position: 4
---

## Common Implementation Patterns

This guide presents several common implementation patterns for the Embrace iOS SDK 7.x that can help you get the most value from mobile observability in your app.
...
class CheckoutCoordinator {
    ...
    func startCheckout() {
        ...
        // You can add attributes when creating the span or after
    ...
    func navigateToShippingScreen() {
        ...
        if let parentSpan = checkoutSpan {
            let shippingSpan = EmbraceIO.shared.createSpan(name: "checkout_shipping", parentSpan: parentSpan)
    ...
    func navigateToPaymentScreen() {
        ...
        // Use if-let to avoid blocking navigation if span tracking fails
        if let parentSpan = checkoutSpan {
            let paymentSpan = EmbraceIO.shared.createSpan(name: "checkout_payment", parentSpan: parentSpan)
    ...
    func completeCheckout(success: Bool) {
        ...
        if success {
            EmbraceIO.shared.log("Checkout completed successfully",
                               severity: .info,
                               attributes: ["order_id": orderId])
        } else {
            EmbraceIO.shared.log("Checkout failed",
                               severity: .warn,
...
extension AppError {
    func logToEmbrace() {
        ...
        EmbraceIO.shared.log(errorMessage,
                            severity: .error,
...
func initializeFeatureFlags() {
    ...
    let span = EmbraceIO.shared.createSpan(name: "feature_flags_initialization")
    ...
    featureFlagSystem.initialize { flags in
        ...
        for (flagName, isEnabled) in flags {
            ...
            if ["new_checkout", "experimental_algorithm"].contains(flagName) {
                EmbraceIO.shared.setProperty(key: "flag_\(flagName)", value: String(isEnabled), lifespan: .session)
            }
        }
...
protocol AnalyticsProvider {
    func log(_ message: String, severity: EmbraceLogSeverity, attributes: [String: String])
    func createSpan(name: String, type: EmbraceType) -> EmbraceSpan?
...
class EmbraceAnalyticsProvider: AnalyticsProvider {
    func log(_ message: String, severity: EmbraceLogSeverity, attributes: [String: String] = [:]) {
        EmbraceIO.shared.log(message, severity: severity, attributes: attributes)
    }

    func createSpan(name: String, type: EmbraceType) -> EmbraceSpan? {
        return EmbraceIO.shared.createSpan(name: name, type: type)
    }

    func addSessionProperty(key: String, value: String, permanent: Bool) {
        EmbraceIO.shared.setProperty(key: key, value: value, lifespan: permanent ? .permanent : .session)
    }
...
class MockAnalyticsProvider: AnalyticsProvider {
    var loggedMessages: [(message: String, severity: EmbraceLogSeverity, attributes: [String: String])] = []
    var createdSpans: [(name: String, type: EmbraceType)] = []
...
class AppFeature {
    ...
    func performAction() {
        let span = analytics.createSpan(name: "perform_action", type: .performance)
        // Do something
...
func performAsyncTask() async throws -> Result {
    ...
    let span = EmbraceIO.shared.createSpan(name: "async_operation")

    do {
        // First step
        span?.addEvent(name: "starting_first_step", attributes: [:])
        let intermediateResult = try await firstStep()
        span?.addEvent(name: "completed_first_step", attributes: [:])

        // Second step
        span?.addEvent(name: "starting_second_step", attributes: [:])
        let finalResult = try await secondStep(intermediateResult)
        span?.addEvent(name: "completed_second_step", attributes: [:])
...
class BackgroundTaskManager {
    func beginBackgroundTask(identifier: String) {
        ...
        EmbraceIO.shared.log("Background task started",
                           severity: .info,
        ...
        backgroundTaskID = UIApplication.shared.beginBackgroundTask(withName: identifier) {
            ...
            EmbraceIO.shared.log("Background task expiring",
                               severity: .warn,
                               attributes: ["task_id": identifier])
    ...
    func endBackgroundTask(identifier: String) {
        ...
        EmbraceIO.shared.log("Background task completed",
                           severity: .info,
                           attributes: ["task_id": identifier])
...
struct CartView: View {
    var body: some View {
        ...
        .onAppear {
            ...
            EmbraceIO.shared.addBreadcrumb("Cart Page Viewed")
        }
    }
}
```
...
struct CheckoutView: View {
    var body: some View {
        ...
        .task {
            ...
            EmbraceIO.shared.addBreadcrumb("Checkout Page Viewed")
        }
    }
}
```
            ...
            .navigationDestination(for: Product.self) { product in
                ProductDetailView(product: product)
                    .onAppear {
                        ...
                        EmbraceIO.shared.addBreadcrumb(
