...
`EmbraceIO.shared` provides methods for managing User Personas.

#### Retrieving Current User Personas

To retrieve the current set of persona tags, use the `getCurrentPersonas(completion:)` method. This fetches the persona tags from the storage that apply to the user at the current point in time.

```swift
EmbraceIO.shared.getCurrentPersonas { personas in
    // use the current personas
}
```

#### Adding User Personas

Personas are plain `String` values, and you can feel free to add your own. Here are some examples on how to add user personas:
...
extension String {
  ...
  static func bucket(name: StaticString) -> String {
    return "bucket_\(name)"
  }
}
```
...
if user.subscriptionTier == .premium {
    EmbraceIO.shared.addPersona("premium_subscriber", lifespan: .permanent)
}

// Tag users who have completed onboarding
if user.hasCompletedOnboarding {
    EmbraceIO.shared.addPersona("completed_onboarding", lifespan: .permanent)
}
```
...
func didEnableDarkMode() {
    EmbraceIO.shared.addPersona("uses_dark_mode", lifespan: .permanent)
}
