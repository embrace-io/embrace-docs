...
- **EmbraceIO** - This is the recommended product to install for quick integration. It is the single public entry point and provides a convenience layer over `EmbraceCore` to simplify the setup interface.
...
- **EmbraceSemantics** - This module contains the public, Embrace-owned signal types (such as `EmbraceSpan` and `EmbraceLog`) as well as constants and attributes used to extend OTel Semantic Conventions.
- **EmbraceMacros** - Swift macros that provide convenience instrumentation helpers.
...
By default, this command installs all the necessary components to use the SDK, including crash reporting and Crashlytics support.
...
you no longer need a separate pod. Crashlytics support is bundled with `EmbraceIO` — enable
